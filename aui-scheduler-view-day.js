...
var SchedulerDayView = A.Component.create({
    ...
    ATTRS: {
        ...
        navigationDateFormatter: {
            value: function(date) {
                ...
                //modified by qy to display current date
                return A.DataType.Date.format(
                    //date, {
                    new Date(), {
                        format: '%A, %B %d, %Y',
    ...
    prototype: {
        ...
        initializer: function() {
            ...
            //added by qy to control the plan/run switch, true is the plan status
            instance.isPlan = true;
        ...
        _dragTickAlignX: function(activeColumn) {
            //added by qy to disable column move
            return;
            var instance = this;
        ...
        _dragTickAlignY: function(event) {
            ...
            //added by qy to disable inactive column move
            /*if ( this.isPlan && colNumber == 1 ){
            	    event.halt();
            	    return;
              }
              if ( !(this.isPlan) && colNumber == 0 ){
            	    event.halt();
            	    return;
              }          
             */
        ...
        _moveCurrentTimeNode: function(time) {
            ...
            var todayColumn = instance.colDaysNode.get('parentNode').first();
        ...
        _prepareEventCreation: function(event, duration) {
            ...
            //added by qy to control plan/run
