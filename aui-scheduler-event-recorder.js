...
var Lang = A.Lang,
    ...
    TPL_BODY_CONTENT = '开始时间<input type="text" name="startDate_time" value="{startDate_time}" />' +
        '<input type="hidden" name="startDate" value="{startDate}" />'  +
        '<br/>结束时间<input type="text" name="endDate_time" value="{endDate_time}" />' +
        '<input type="hidden" name="endDate" value="{endDate}" />' +
        //modified by qy
        '<BR/><BR/><input type="checkbox" name="isFixed" value="true" {isFixedCheck} />固定时间日程' + 
        '<BR/><input type="checkbox" name="isDone" value="true" {isDoneCheck} />已完成',
...
var SchedulerEventRecorder = A.Component.create({
    ...
    prototype: {
        ...
        getTemplateData: function() {
            ...
            var hh_startDate = "";
            if( evt.get('startDate').getHours() > 9 ) hh_startDate = ""+evt.get('startDate').getHours();
            else hh_startDate = "0"+ evt.get('startDate').getHours();
            if( evt.get('startDate').getMinutes() > 9 ) hh_startDate += ":"+evt.get('startDate').getMinutes();
            else hh_startDate += ":0"+ evt.get('startDate').getMinutes();

            var hh_endDate = "";
            
            if( evt.get('endDate').getHours() > 9 ) hh_endDate = ""+evt.get('endDate').getHours();
            else hh_endDate = "0"+ evt.get('endDate').getHours();
            if( evt.get('endDate').getMinutes() > 9 ) hh_endDate += ":"+evt.get('endDate').getMinutes();
            else hh_endDate += ":0"+ evt.get('endDate').getMinutes();
            
            var myendDate = evt.get('endDate');
            if(evt.get('endDate').getDate() > evt.get('startDate').getDate() ){
            	  hh_endDate = "23:59";
            	  myendDate = new Date(myendDate.getFullYear(),myendDate.getMonth(),evt.get('startDate').getDate(), 23,59,00);
            }
            ...
            return {
                ...
                //modified by qingyi
                startDate_time: hh_startDate,
                endDate_time: hh_endDate,
                isFixedCheck: evt.get('isFixed')?"checked":"",
                isDoneCheck: evt.get('isDone')?"checked":"",
        ...
        getUpdatedSchedulerEvent: function(optAttrMap) {
            var instance = this,
                ...
                //will be updated later by qy
                formValues = instance.serializeForm();
                var s = new Date(formValues.startDate);
                var ss_time = formValues.startDate_time;
                var ss_pos = ss_time.indexOf(":");
                var t_hour = ss_time.substring(0,ss_pos);
                var t_min = ss_time.substring(ss_pos+1,ss_time.length);
                formValues.startDate = (new Date(s.getFullYear(),s.getMonth(),s.getDate(),t_hour,t_min)).getTime();
                s = new Date(formValues.endDate);
                ss_time = formValues.endDate_time;
                ss_pos = ss_time.indexOf(":");
                t_hour = ss_time.substring(0,ss_pos);
                t_min = ss_time.substring(ss_pos+1,ss_time.length);
                formValues.endDate = (new Date(s.getFullYear(),s.getMonth(),s.getDate(),t_hour,t_min)).getTime();
                formValues.isFixed = formValues.isFixed? true:false;
                formValues.isDone = formValues.isDone? true:false;
        ...
        _onClickSchedulerEvent: function(event) {
            ...
            //added by qy to disable inactive column
            var colNumber = event.currentTarget._node.parentNode.parentNode.getAttribute('data-colnumber');
                
            var isPlan = instance.get('scheduler').viewStack.day.isPlan;

            if(( isPlan && colNumber == 0 ) || ( !isPlan && colNumber == 1 )) {
